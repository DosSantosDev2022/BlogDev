    ...
    <NavigationMenu.Root className="lg:col-span-3  w-full flex md:flex-row flex-col justify-around gap-10">
      <NavigationMenu.List className="flex flex-col gap-2">
        ...
        {Links.map((link) => (
          <NavigationMenu.Item key={link.id}>
            <Link className="text-slate-50 hover:underline" href={link.url}>
              {link.text}
            </Link>
          </NavigationMenu.Item>
        ))}
      ...
      <NavigationMenu.List className="flex flex-col gap-2">
        ...
        {Links2.map((link2) => (
          <NavigationMenu.Item key={link2.id}>
            <Link className="text-slate-50 hover:underline" href={link2.url}>
              {link2.text}
            </Link>
          </NavigationMenu.Item>
        ))}
      ...
      <NavigationMenu.List className="flex flex-col gap-2">
        ...
        {social.map((link) => (
          <NavigationMenu.Item key={link.id}>
            <Link
              className="text-slate-50 flex items-center gap-3 hover:underline"
              href={link.url}
            >
